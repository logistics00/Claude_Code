  ...
  if (node.marks) {
    for (const mark of node.marks) {
      switch (mark.type) {
        ...
        case "code":
          content = (
            <code key="code" className="rounded bg-gray-100 px-1 py-0.5 font-mono text-sm dark:bg-gray-800">
              {content}
            </code>
          );
          break;
...
function renderNode(node: ContentNode, index: number): React.ReactNode {
  ...
  switch (node.type) {
    ...
    case "heading": {
      ...
      }[level] || "mb-2 mt-4 text-lg font-semibold text-gray-900 dark:text-white";
    ...
    case "bulletList":
      return (
        <ul key={key} className="mb-4 ml-6 list-disc space-y-1 text-gray-700 dark:text-gray-300">
          {node.content?.map((child, i) => renderNode(child, i))}
        </ul>
      );
    ...
    case "orderedList":
      return (
        <ol key={key} className="mb-4 ml-6 list-decimal space-y-1 text-gray-700 dark:text-gray-300">
          {node.content?.map((child, i) => renderNode(child, i))}
        </ol>
      );
    ...
    case "codeBlock":
      return (
        <pre key={key} className="mb-4 overflow-x-auto rounded-lg bg-gray-900 p-4 text-sm text-gray-100">
          <code>{node.content?.map((child) => (child as TextNode).text).join("")}</code>
        </pre>
      );
    ...
    case "blockquote":
      return (
        <blockquote key={key} className="mb-4 border-l-4 border-gray-300 pl-4 italic text-gray-600 dark:border-gray-600 dark:text-gray-400">
          {node.content?.map((child, i) => renderNode(child, i))}
        </blockquote>
      );
...
export function NoteRenderer({ content }: { content: string }) {
  ...
  return <div className="prose-custom">{doc.content.map((node, i) => renderNode(node, i))}</div>;
}
