...
export function validateContentJson(contentJson: string): {
    valid: boolean;
    error?: string;
} {
...
export function createNote(userId: string, title: string, contentJson: string): string {
...
export function getNotesByUserId(userId: string): Note[] {
    return query<Note>(`SELECT * FROM notes WHERE user_id = ? ORDER BY updated_at DESC`, [
        userId,
    ]);
}
...
export function getPublicNoteBySlug(slug: string): Note | undefined {
    return get<Note>(`SELECT * FROM notes WHERE public_slug = ? AND is_public = 1`, [
        slug,
    ]);
}
